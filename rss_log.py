...
while i < len(items):
    if sql.query('SELECT * FROM bbcfeeds WHERE url=' + items[i]['link'] + ' AND  date=' + items[i]['published'] ) == '':
      sql.query('INSERT INTO news_log ' + items[i]['link'] + ',' + items[i] + ',' + items[i]['summary'] + ',' + (len(items) - i) * 4)
    else:
        curpoints = sql.query('SELECT FROM points WHERE url = ' + items[i]['link'])
        sql.query('UPDATE url = ' + items[i]['link'] + ' INSERT points ' + intcurpoints + curpoints * 1.002)
    i = i + 1
